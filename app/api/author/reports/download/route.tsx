...
export async function GET(req: Request) {
  try {
    ...
    const pdfBuffer = await buildPdf({
      title: 'Sales Report',
      subtitle: `Author — ${user.author.name}`,
      summary: [
    ...
    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="author-report-${new Date().toISOString().split('T')[0]}.pdf"`,
      },
    });
  } catch (error) {
    console.error('Author PDF error:', error);
