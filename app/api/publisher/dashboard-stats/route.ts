  ...
  try {
    ...
    books.forEach(b => {
      b.purchaseItems.forEach(item => {
        ...
        if (date >= weekStart) {
          weekMap[key] = (weekMap[key] || 0) + Number(item.priceAtPurchase);
        }
      });
    });
    const weeklyChart = weekDays.map(day => ({
      day,
      revenue: weekMap[day] || 0,
    }));
    ...
    books.forEach(b => {
      b.purchaseItems.forEach(item => {
        ...
        const key = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
        if (date >= yearStart) {
          yearMap[key] = (yearMap[key] || 0) + Number(item.priceAtPurchase);
        }
      });
    });
    const yearlyChart = yearMonths.map(month => ({
      month,
      revenue: yearMap[month] || 0,
    }));
